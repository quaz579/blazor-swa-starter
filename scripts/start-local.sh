#!/usr/bin/env bash
# Starts Azurite, the Functions API host, the Blazor dev server, and the SWA
# CLI proxy for interactive local development. Idempotent: a service already
# listening on its port is left alone. Detaches on success; use stop-local.sh
# to tear down what this script started.
set -euo pipefail
...
AZURITE_DATA_DIR="$REPO_ROOT/.azurite/local"
PIDS_FILE="$RUN_DIR/local.pids"
...
if [ -n "$(listener_pid "$AZURITE_BLOB_PORT")" ]; then
  ...
  npx --yes "azurite@^3.37.0" --silent \
    ...
    > "$LOG_DIR/azurite-local.log" 2>&1 &
  azurite_pid=$!
  record_pid "azurite" "$azurite_pid" "$AZURITE_BLOB_PORT"
  wait_for "Azurite" 30 "$LOG_DIR/azurite-local.log" port_responds "http://127.0.0.1:$AZURITE_BLOB_PORT/"
...
if [ -n "$(listener_pid "$API_PORT")" ]; then
  ...
  (cd "$REPO_ROOT/src/App.Api" && exec func start --port "$API_PORT") > "$LOG_DIR/func-local.log" 2>&1 &
  func_pid=$!
  record_pid "func" "$func_pid" "$API_PORT"
  wait_for "Functions API" 60 "$LOG_DIR/func-local.log" http_ok "http://localhost:$API_PORT/api/health"
...
if [ -n "$(listener_pid "$WEB_PORT")" ]; then
  ...
  (cd "$REPO_ROOT/src/App.Web" && exec dotnet run --urls "http://localhost:$WEB_PORT") > "$LOG_DIR/web-local.log" 2>&1 &
  web_pid=$!
  record_pid "web" "$web_pid" "$WEB_PORT"
  wait_for "Blazor dev server" 60 "$LOG_DIR/web-local.log" http_ok "http://localhost:$WEB_PORT/"
...
if [ -n "$(listener_pid "$SWA_PORT")" ]; then
  ...
  (cd "$REPO_ROOT" && exec swa start) > "$LOG_DIR/swa-local.log" 2>&1 &
  swa_pid=$!
  record_pid "swa" "$swa_pid" "$SWA_PORT"
  wait_for "SWA CLI" 30 "$LOG_DIR/swa-local.log" http_ok "http://localhost:$SWA_PORT/"
  echo "SWA CLI started (pid $swa_pid) - http://localhost:$SWA_PORT"
fi

echo ""
echo "Local dev environment ready. Open http://localhost:$SWA_PORT"
echo "Stop with: $SCRIPT_DIR/stop-local.sh"
