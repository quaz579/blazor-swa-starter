...
cached — and starts nothing itself. **Known false-fail on macOS:** the last check globs both
`~/Library/Caches/ms-playwright` and `~/.cache/ms-playwright`; on a Mac only the first exists,
`find` exits non-zero for the missing path, and the script's `set -o pipefail` turns that into a
reported `FAIL` even when Chromium is actually installed. If you hit exactly that failure, confirm
directly with `ls ~/Library/Caches/ms-playwright | grep chromium` before assuming Playwright
isn't installed.
