#!/usr/bin/env bash
# Stops only the processes scripts/start-e2e.sh recorded as its own. Safe to
...
PIDS_FILE="$REPO_ROOT/.azurite/run/e2e.pids"

echo "Stopping E2E test environment..."

if [ ! -f "$PIDS_FILE" ]; then
  echo "No e2e.pids file found, nothing to stop"
  exit 0
fi
