...
You own target framework monikers, NuGet package versions, `global.json`, and the triage of
breaking changes that a version bump surfaces. You do not own literal framework strings living
in infra/config/docs files — `swa-infra` owns those. When you run the repo-wide sweep for stale
framework literals, only fix hits inside `*.csproj` and `global.json`; report every other hit to
`swa-infra` rather than editing it yourself, so the two agents don't collide on the same files.
